...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      let user = email ? await User.findOne({ email }) : await User.findOne({ googleEmail });
      ...
      if (user.password) {   // if signing in with password, hash the password, if token with google, hash the token
